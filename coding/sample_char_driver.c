...
static irqreturn_t irq_handler (int irq, void *dev_id) { spin_lock_irq (&chr_spinlock);
{
	chr_spinlock_variable++;
...
static ssize_t chr_read(struct file *filp, char_user *buf, size_t len, loff_t *off);
...
struct kobj_attribute sysfs_attr = ATTR (sysfs_val, 0660, sysfs_show, sysfs_store);

int thrd_funcl (void *p);
...
int thrd_funcl (void *p)
{
	...
	while(!kthread_should_stop()) {
		if (!spin_is_locked (&chr_spinlock)) {
			//printk (KERN_INFO" spinlock is not locked in thread functionl\n");
		}
...
static ssize_t chr_write(struct file *filp, const char _user *buf, size_t len, loff_t *off)
{
	printk (KERN_INFO "Write Function\n"); 
	return 0;
}

static int _init chr_driver_init (void)
{
	...
	tasklet = kmalloc(sizeof(struct tasklet_struct), GFP KERNEL); 
	if (tasklet == NULL) {
	...
	chr_threadl = kthread_create (thrd_func1, NULL, "chr thread 1");
	if (chr_thread1) {
...
void exit chr_driver_exit (void)
{
